  ...
  if ($expiryDate.AddDays($GracePeriod) -le $currentDate) {
    ...
    try {
      if ($DisableSubscription) {
        ...
        Write-Host "`u{2713} Subscription $($Subscription.name) is now disabled! You have 90days to recover disabled subscriptions via Support Ticket." -ForegroundColor Green
      }
      else {
        Write-Host "`u{2713} DisableSubscription parameter set to false, skipping subscription cancellation." -ForegroundColor Green
      }
    }
    catch {
